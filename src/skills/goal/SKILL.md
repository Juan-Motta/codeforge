...
under `owner=goal` (`.codeforge/rules/execution.md`) — it never re-implements them — advancing the state
machine in `.codeforge/workflow/state.md`. All line/state schemas are in `.codeforge/rules/goal-state.md`.
...
1. **Classify.** If the objective is a **bug**, STOP and route the human to `/fix-bug` — v1 `/goal`
   is feature-only.
2. **Clean slot** (`.codeforge/rules/execution.md`): if `.codeforge/workflow/state.md` has a non-`done`
...
on Claude; a plain prompt elsewhere). On approval write `gate1` (`.codeforge/rules/goal-state.md`) and
...
1. `base=$(sh .codeforge/scripts/goal-state.sh field base_sha)`; confirm
   `sh .codeforge/scripts/goal-digest.sh "$base" .` == the certification digest, AND
   `sh .codeforge/scripts/check-gates.sh` is green. (PowerShell: `pwsh .codeforge/scripts/goal-digest.ps1 "$base" .`.)
2. Write the CHANGELOG (digest-neutral).
3. **Single commit** (stage the digest-covered set ∪ CHANGELOG + the E2E report).
4. Assert `sh .codeforge/scripts/goal-digest.sh "$base" . --from-head` == the certification digest AND
...
**Recovery:** on a ship-side `check-gates` red, `sh .codeforge/scripts/goal-state.sh ship-red-bump`;
...
`## /goal loop` is the source of truth (`.codeforge/rules/goal-state.md`). On a mid-session compaction,
