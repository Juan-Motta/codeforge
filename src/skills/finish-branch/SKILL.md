...
```sh
sh .codeforge/scripts/check-gates.sh        # pwsh .codeforge/scripts/check-gates.ps1 on Windows
```

It reads `.codeforge/workflow/state.md`, confirms **every box for the active profile** is checked (or
N/A), and exits non-zero listing any that aren't (`.codeforge/rules/ship-gates.md`). If it fails,
...
Verified / Attested / Advisory distinction in `.codeforge/rules/ship-gates.md`.
...
- Add a newest-first entry to `docs/CHANGELOG.md` (`.codeforge/rules/docs-layout.md`).
- Save any reusable learning per `.codeforge/rules/memory.md` (solved bugs → `docs/solutions/`,
  decisions → `docs/adr/`).
...
a gate — `.codeforge/rules/ship-gates.md`) — approve only because you just confirmed the gates
in step 1. Write a PR description stating what changed, why, and how it was verified.

## 6. Update transient state

Only now — after shipping — record the PR link / merge outcome in `.codeforge/workflow/state.md` so
...
| "The push prompt appeared, so I'm cleared to ship." | The prompt is a commit-confirmation that reads no gate state (`.codeforge/rules/ship-gates.md`). It proves nothing — you are the gate. |
| "I'll record the PR outcome later." | Update `.codeforge/workflow/state.md` with the PR link / merge outcome now, so the workflow reflects reality. |

## Red flags

- An open box in `.codeforge/workflow/state.md` and you're pushing anyway.
...
- [ ] Every required box for the active profile is checked in `.codeforge/workflow/state.md`.
