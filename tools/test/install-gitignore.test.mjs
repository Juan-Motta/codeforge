...
function countWorkflowIgnores(gitignorePath) {
  return readFileSync(gitignorePath, 'utf8').split('\n').filter((l) => l.trim() === '.codeforge/workflow/').length;
}

// Strict (untrimmed) count — used where the fixture deliberately seeds near-miss lines (leading
// space, wrong case) that would trim-equal '.codeforge/workflow/' and so must NOT be conflated with it.
// A trailing CR is stripped (PowerShell Add-Content appends CRLF on Windows, so the legit line
// is '.codeforge/workflow/\r') — but nothing else, so a leading-space/wrong-case near-miss still fails.
function countExactWorkflowLines(gitignorePath) {
  return readFileSync(gitignorePath, 'utf8').split('\n').filter((l) => l.replace(/\r$/, '') === '.codeforge/workflow/').length;
}
...
// .codeforge/workflow/ anyway, hiding the fix).
...
test('install.sh restores .codeforge/workflow/ when the marker exists but the entry was removed', () => {
  ...
  try {
    ...
    assert.equal(countWorkflowIgnores(join(target, '.gitignore')), 1, '.codeforge/workflow/ present exactly once');
  } finally { rmSync(target, { recursive: true, force: true }); }
});

test('install.sh fresh install then re-install yields .codeforge/workflow/ exactly once (no double-append)', () => {
  ...
  try {
    ...
    assert.equal(countWorkflowIgnores(join(target, '.gitignore')), 1, 'no duplicate .codeforge/workflow/ across installs');
  } finally { rmSync(target, { recursive: true, force: true }); }
});

test('install.sh: no-trailing-newline seed gets .codeforge/workflow/ on its own line, exactly once', () => {
  const target = mkdtempSync(join(tmpdir(), 'cf-gi-nonl-'));
  try {
    // Marker present, .codeforge/workflow/ absent, and the file's last byte is NOT a newline — proves the
    // append is newline-safe (doesn't fuse .codeforge/workflow/ onto the previous line).
    ...
    assert.ok(lines.includes('.codeforge/workflow/'), '.codeforge/workflow/ appears as its own line (not fused)');
    assert.equal(countWorkflowIgnores(join(target, '.gitignore')), 1, '.codeforge/workflow/ present exactly once');
    // Guard against fusion explicitly: no line should contain .codeforge/workflow/ glued onto other content.
    for (const line of lines) {
      if (line.includes('.codeforge/workflow/')) {
        assert.equal(line.trim(), '.codeforge/workflow/', `.codeforge/workflow/ must not be fused onto other content: ${JSON.stringify(line)}`);
      }
    }
  } finally { rmSync(target, { recursive: true, force: true }); }
});

test('install.ps1 restores .codeforge/workflow/ when the marker exists but the entry was removed', { skip: hasPwsh ? false : 'pwsh not available' }, () => {
  ...
  try {
    ...
    assert.equal(countWorkflowIgnores(join(target, '.gitignore')), 1, '.codeforge/workflow/ present exactly once (ps1)');
  } finally { rmSync(target, { recursive: true, force: true }); }
});
...
test('install.ps1: near-miss lines (leading space / wrong case) do not suppress the exact entry', { skip: hasPwsh ? false : 'pwsh not available' }, () => {
  ...
  try {
    ...
    // treat as "already ignored" — but no EXACT `.codeforge/workflow/` line. The `-ceq` guard must still append it.
    const seed = codeforgeMarkerPs1() + '\n.DS_Store\n .codeforge/workflow/\n.WORKFLOW/\n.claude/settings.local.json\n';
    writeFileSync(join(target, '.gitignore'), seed);
    execFileSync('pwsh', ['-File', join(REPO, 'install.ps1'), target], { cwd: REPO, stdio: 'pipe' });
    const contents = readFileSync(join(target, '.gitignore'), 'utf8');
    assert.ok(contents.includes(' .codeforge/workflow/'), 'the leading-space near-miss line is left untouched');
    assert.ok(contents.includes('.WORKFLOW/'), 'the wrong-case near-miss line is left untouched');
    assert.equal(countExactWorkflowLines(join(target, '.gitignore')), 1, 'exact .codeforge/workflow/ appended despite near-miss lines');
  } finally { rmSync(target, { recursive: true, force: true }); }
});

test('install.ps1 fresh install then re-install yields .codeforge/workflow/ exactly once (no double-append)', { skip: hasPwsh ? false : 'pwsh not available' }, () => {
  ...
  try {
    ...
    assert.equal(countWorkflowIgnores(join(target, '.gitignore')), 1, 'no duplicate .codeforge/workflow/ across ps1 installs');
  } finally { rmSync(target, { recursive: true, force: true }); }
});
